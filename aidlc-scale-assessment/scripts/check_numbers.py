...
def main() -> int:
    ...
    if not with_closes or not with_aidlc:
        ...
        if carry:
            print(f"\n⚠ 이월(`construction`)로 내려간 Contract {len(carry)}건:"
                  f" {', '.join(carry[:20])} — 파트 4 §3 에 **되돌림 위험**이 적혀 있는지"
    ...
    # 파트 4 §2 는 `prep` 22건을 *"착수 후에는 닫힐 수 없다"* 고 적는다 — 채점자가 **논리
    ...
    # ── ④-i 파트 3 §4 「판정」 열 ↔ 그 행 Contract 의 정본 (it.18 신설) ──────
    ...
    if not jd_pop:
        print("⚠ **이 축은 꺼졌다 — 0건이 통과가 아니다.** 판정 라벨 칸과 ID 칸이 같은 행에"
              " 있는 표를 찾지 못했다. 파트 3 §4 표가 **원문 항목 | 판정 | 근거 | Contract**"
    ...
    unchecked = [
        ...
        " → **파트 3 §4 의 세 묶음 수와 그것을 인용한 서술을 손으로 대 본다.**"
        ...
        " **it.18 에서 한 열만 갈라 채택했다** — 파트 3 §4 의 판정 열은 셀 텍스트가 정본 표시"
